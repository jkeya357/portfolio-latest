...
const ProjectCard = ({project}) => {
    ...
    const {title, description, image, technologies, metrics, demo, githubUrl} = project
  ...
  return (
    <div className='group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-All duration-300'>
        <div className='relative h-64 overflow-hidden'>
            ...
            <div className='absolute inset-0 bg-linear-0-to-t  from-black via-black/60 to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-300'/>
        ...
        <div className='p-6 space-y-4'>
            ...
            <div className='flex flex-wrap gap-2'>
                {technologies.map((tech, index) => (
                    <span
                        key={index}
                        className='px-3 py-1 test-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors duration-200'
                    >{tech}</span>
                ))}
            </div>
